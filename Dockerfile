...
RUN powershell iwr 'https://dot.net/v1/dotnet-install.ps1' -OutFile './dotnet-install.ps1' && \
    powershell ./dotnet-install.ps1 -InstallDir 'C:/dotnet-sdk' -Channel Current -Version %ARG_DOTNET_SDK_VERSION% && \
    setx /M PATH "%PATH%;C:\dotnet-sdk" && \
	...
	setx /M PATH "%PATH%;C:\dotnet-tools" && \
